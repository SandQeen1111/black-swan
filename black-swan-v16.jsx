...
// ════════════════════════════════════════════════════════════════════
//  B L A C K  S W A N  v16 — OMEGA EDITION  ★ COMPLETE ROADMAP ★
//  👥 Multi · 🌐 Shader · 🖐️ Körper · 🧠 KI · 🧊 3D · ⚛️ Physik · 🎭 Partikel · 🎮 Game
// ════════════════════════════════════════════════════════════════════

const KW={wenn:"if",sonst_wenn:"elif",sonst:"else",solange:"while","für":"for",
abbrechen:"break",weiter:"continue",übergeben:"pass",funktion:"def",
zurückgeben:"return",klasse:"class",selbst:"self",wahr:"True",falsch:"False",
und:"and",oder:"or",nicht:"not",ist:"is",nichts:"None",versuche:"try",
ausnahme:"except",schließlich:"finally",importiere:"import",
von:"from",als:"as",mit:"with",anonym:"lambda","in":"in"};
const BF={drucke:"print",bereich:"range",länge:"len",ganzzahl:"int",
dezimal:"float",zeichenkette:"str",liste:"list",abs_wert:"abs",
runden:"round",sortiert:"sorted",minimum:"min",maximum:"max",summe:"sum"};
const ALL={...KW,...BF};
const MT={".anhängen(":".append(",".herausholen(":".pop(",".sortieren(":".sort(",
".umkehren(":".reverse(",".ersetzen(":".replace(",".teilen(":".split(",
".einfügen(":".insert(",".entfernen(":".remove(",".großbuchstaben(":".upper(",
".kleinbuchstaben(":".lower(",".streifen(":".strip(",".finden(":".find(",
".zählen(":".count(",".verbinden(":".join(",".beginnt_mit(":".startswith(",
".endet_mit(":".endswith(",".schlüssel(":".keys(",".werte(":".values("};
...
const WB_S="(?<![a-zA-ZäöüÄÖÜß_0-9])",WB_E="(?![a-zA-ZäöüÄÖÜß_0-9])";
...
const isCont=/^(sonst_wenn\s|sonst:|ausnahme|schließlich:)/.test(text);
...
if((m=l.match(/^für\s+([\wäöüÄÖÜß]+)\s+in\s+(.+):$/)))return"for(let "+m[1]+" of "+tEx(m[2])+"){";
...
.replace(/\bnicht\s+/g,"!").replace(/\.anhängen\(/g,".push(")
...
.replace(/\.einfügen\(/g,".__einfügen(").replace(/\.entfernen\(/g,".__entfernen(")
.replace(/\.großbuchstaben\(\)/g,".toUpperCase()").replace(/\.kleinbuchstaben\(\)/g,".toLowerCase()")
...
function länge(x){return x.length!=null?x.length:Object.keys(x).length}
...
function wörterbuch(pairs){if(!pairs)return{};const d={};for(const p of pairs)d[p[0]]=p[1];return d;}
...
function fülle(c){__d.push({t:"f",c})}
Array.prototype.__einfügen=function(pos,item){this.splice(pos,0,item)};
...
const schildkröte={
  vorwärts(d){const r=__turtle.a*Math.PI/180;const nx=__turtle.x+d*Math.cos(r);const ny=__turtle.y+d*Math.sin(r);
  ...
  rückwärts(d){schildkröte.vorwärts(-d)},
...
const PHYSICS_RT = `
const __körper_liste=[];let __schwerkraft_y=0.3,__schwerkraft_x=0;
function schwerkraft(sy,sx){__schwerkraft_y=sy;if(sx!==undefined)__schwerkraft_x=sx;}
function körper(x,y,b,h,opt){
  const k={x,y,b:b||20,h:h||20,vx:0,vy:0,fest:false,farbe:"#f43f5e",rückprall:0.7,reibung:0.99,id:__körper_liste.length};
  if(opt){if(opt.fest!==undefined)k.fest=opt.fest;if(opt.farbe)k.farbe=opt.farbe;
    if(opt.rückprall!==undefined)k.rückprall=opt.rückprall;if(opt.vx!==undefined)k.vx=opt.vx;if(opt.vy!==undefined)k.vy=opt.vy;}
  __körper_liste.push(k);return k;}
function kraft(k,fx,fy){if(!k.fest){k.vx+=fx;k.vy+=fy;}}
function physik_schritt(){
  for(const k of __körper_liste){if(k.fest)continue;k.vx+=__schwerkraft_x;k.vy+=__schwerkraft_y;k.vx*=k.reibung;k.vy*=k.reibung;
    k.x+=k.vx;k.y+=k.vy;
    if(k.x<0){k.x=0;k.vx*=-k.rückprall;}if(k.x+k.b>400){k.x=400-k.b;k.vx*=-k.rückprall;}
    if(k.y+k.h>400){k.y=400-k.h;k.vy*=-k.rückprall;}if(k.y<0){k.y=0;k.vy*=-k.rückprall;}}
  for(let i=0;i<__körper_liste.length;i++){for(let j=i+1;j<__körper_liste.length;j++){
    const a=__körper_liste[i],b=__körper_liste[j];
    if(a.x<b.x+b.b&&a.x+a.b>b.x&&a.y<b.y+b.h&&a.y+a.h>b.y){
      if(a.fest&&!b.fest){b.vy*=-b.rückprall;b.y=a.y-b.h;}
      else if(!a.fest&&b.fest){a.vy*=-a.rückprall;a.y=b.y-a.h;}
...
function körper_zeichnen(k){__d.push({t:"r",x:k.x,y:k.y,w:k.b,h:k.h,c:k.farbe});}
function alle_körper_zeichnen(){for(const k of __körper_liste)körper_zeichnen(k);}`;

const PARTICLE_RT = `
const __partikel=[];
function partikel_erzeugen(x,y,opt){const n=opt?.anzahl||20,sp=opt?.geschwindigkeit||3,f=opt?.farbe||"#facc15",l=opt?.leben||60,g=opt?.größe||3;
  for(let i=0;i<n;i++){const a=Math.random()*Math.PI*2;const v=Math.random()*sp+sp*0.2;
    __partikel.push({x,y,vx:Math.cos(a)*v,vy:Math.sin(a)*v,leben:l,maxL:l,farbe:f,größe:g+Math.random()*2});}}
...
function partikel_regen(y,farbe,anzahl){for(let i=0;i<(anzahl||5);i++){
  __partikel.push({x:Math.random()*400,y:y||0,vx:(Math.random()-0.5)*0.5,vy:Math.random()*2+1,leben:120,maxL:120,farbe:farbe||"#38bdf8",größe:2+Math.random()*2});}}
function partikel_aktualisieren(){for(let i=__partikel.length-1;i>=0;i--){
  ...
  else{const a=p.leben/p.maxL;__d.push({t:"c",x:p.x,y:p.y,r:p.größe*a,c:p.farbe});}}}
function partikel_anzahl(){return __partikel.length;}`;

const GAME_RT = `
function taste(t){return __keys[t.toLowerCase()]||__keys[t]||false;}
function punkte_anzeigen(x,y,p,opt){__d.push({t:"tx",x,y,txt:String(p),c:opt?.farbe||"#facc15",s:opt?.größe||20});}
...
// ══════════════════════════════════════════════════════════════════════
//  🖐️ BODY TRACKING RUNTIME — Webcam-basierte Bewegungserkennung
// ══════════════════════════════════════════════════════════════════════
const CAMERA_RT = `
...
// ══════════════════════════════════════════════════════════════════════
//  🧠 KI TRAINING RUNTIME — Trainierbare KI auf Deutsch
// ══════════════════════════════════════════════════════════════════════
const KI_RT = `
...
// ══════════════════════════════════════════════════════════════════════
//  🧊 3D RUNTIME — Three.js mit deutscher API
// ══════════════════════════════════════════════════════════════════════
const THREE_RT = `
let __3d_mode=false;
...
function würfel_3d(x,y,z,größe,farbe){__d3.push({t:"box",x:x||0,y:y||0,z:z||0,s:größe||1,c:farbe||"#f43f5e"});}
function kugel_3d(x,y,z,radius,farbe){__d3.push({t:"sphere",x:x||0,y:y||0,z:z||0,r:radius||0.5,c:farbe||"#38bdf8"});}
function zylinder_3d(x,y,z,radius,höhe,farbe){__d3.push({t:"cyl",x:x||0,y:y||0,z:z||0,r:radius||0.5,h:höhe||1,c:farbe||"#2dd4bf"});}
function ebene_3d(x,y,z,b,h,farbe){__d3.push({t:"plane",x:x||0,y:y||0,z:z||0,w:b||10,h:h||10,c:farbe||"#1a1a2e"});}
function licht_3d(x,y,z,farbe,stärke){__d3.push({t:"light",x:x||5,y:y||5,z:z||5,c:farbe||"#ffffff",i:stärke||1});}
...
function text_3d(x,y,z,txt,farbe,größe){__d3.push({t:"txt3d",x:x||0,y:y||0,z:z||0,txt:String(txt),c:farbe||"#fff",s:größe||1});}
`;

// ══════════════════════════════════════════════════════════════════════
//  👥 MULTIPLAYER RUNTIME — Shared state via storage API
// ══════════════════════════════════════════════════════════════════════
const MULTI_RT = `
...
function mehrspieler_senden(schlüssel,wert){
  __mp_daten[schlüssel]=wert;
  if(__storage){try{__storage.set("mp:"+__mp_id,JSON.stringify({id:__mp_id,t:Date.now(),d:__mp_daten}),true)}catch(e){}}
}
function mehrspieler_lesen(spieler,schlüssel){
  if(__mp_alle[spieler]&&__mp_alle[spieler].d)return __mp_alle[spieler].d[schlüssel];
  return null;
}
...
// ══════════════════════════════════════════════════════════════════════
//  🌐 SHADER RUNTIME — WebGL Fragment-Shader mit deutscher API
// ══════════════════════════════════════════════════════════════════════
const SHADER_RT = `
...
// ── HIGHLIGHTING ─────────────────────────────────────────────────────
const KW_RE="wenn|sonst_wenn|sonst|solange|für|abbrechen|weiter|übergeben|funktion|zurückgeben|klasse|selbst|wahr|falsch|und|oder|nicht|nichts|versuche|ausnahme|schließlich|in";
const FN_RE="drucke|bereich|länge|zeichenkette|ganzzahl|dezimal|liste|sortiert|minimum|maximum|summe|zufall|zufall_dezimal|sinus|kosinus|wurzel|potenz|abs_wert|runden|typ|formatieren|wörterbuch|zeichne_kreis|zeichne_linie|zeichne_rechteck|zeichne_text|fülle|spiele_ton|spiele_note|schildkröte|schwerkraft|körper|kraft|physik_schritt|kollision|körper_zeichnen|alle_körper_zeichnen|partikel_erzeugen|partikel_explosion|partikel_regen|partikel_aktualisieren|partikel_anzahl|taste|spielobjekt|objekt_zeichnen|objekt_kreis|objekte_kollidieren|abstand|bewege|begrenze|punkte_anzeigen|kamera_starten|kamera_stoppen|ki_erstellen|ki_lernen|ki_vorhersagen|ki_genauigkeit|ki_daten_anzahl|ki_klassen|szene_3d|würfel_3d|kugel_3d|zylinder_3d|ebene_3d|licht_3d|kamera_3d|blickpunkt_3d|drehen_3d|text_3d|mehrspieler_name|mehrspieler_senden|mehrspieler_lesen|mehrspieler_alle|mehrspieler_anzahl|mein_name|shader|shader_wert";
...
.replace(new RegExp(`(?<![a-zA-ZäöüÄÖÜß_])(${KW_RE})(?![a-zA-ZäöüÄÖÜß_])`,"g"),'<b style="color:#f43f5e">$1</b>')
.replace(new RegExp(`(?<![a-zA-ZäöüÄÖÜß_.])(${FN_RE})(?=[.(])`,"g"),'<b style="color:#38bdf8">$1</b>')}

// ── AI PROMPTS ───────────────────────────────────────────────────────
const AI_SYS=`Du bist Black Swan — die deutsche Programmiersprache. Generiere NUR Code, KEIN Markdown/Backticks.
Keywords: wenn, sonst, sonst_wenn, für, solange, funktion, zurückgeben, wahr, falsch, und, oder, nicht
Funktionen: drucke(), bereich(), länge(), zeichenkette(), ganzzahl(), zufall(min,max), sinus(), kosinus(), PI
2D Canvas: zeichne_kreis(x,y,r,farbe), zeichne_linie(x1,y1,x2,y2,farbe,breite), zeichne_rechteck(x,y,b,h,farbe), zeichne_text(x,y,text,farbe,größe), fülle(farbe)
Schildkröte: schildkröte.vorwärts(d), .links(w), .rechts(w), .farbe(f), .breite(b), .kreis(r)
Physik: schwerkraft(stärke), körper(x,y,b,h,{fest,farbe,rückprall}), kraft(k,fx,fy), physik_schritt(), alle_körper_zeichnen()
...
3D: szene_3d(), würfel_3d(x,y,z,größe,farbe), kugel_3d(x,y,z,radius,farbe), zylinder_3d(x,y,z,r,h,farbe), ebene_3d(x,y,z,b,h,farbe), licht_3d(x,y,z), kamera_3d(x,y,z), drehen_3d(rx,ry,rz)
...
Mehrspieler: mehrspieler_name(name), mehrspieler_senden(schlüssel,wert), mehrspieler_lesen(spieler,schlüssel), mehrspieler_alle(), mehrspieler_anzahl(), mein_name()
Shader: shader(glsl_code), shader_wert(name,wert) — GLSL Fragment-Shader, uniforms: zeit, auflösung, maus
Animation: funktion zeichne(): für 60fps. Nutze rahmen, maus_x, maus_y, maus_gedrückt
Sound: spiele_ton(freq,dauer), spiele_note("C4",dauer)
Alles auf Deutsch.`;

const TUTOR_SYS=`Du bist der Black Swan KI-Tutor. Erkläre Code auf Deutsch in einfacher Sprache. Max 4-5 Sätze. Sei ermutigend.
Black Swan ist eine deutsche Programmiersprache die zu Python transpiliert. für=for, wenn=if, funktion=def, drucke=print.
Neue Features: 3D (szene_3d, würfel_3d, kugel_3d), Physik (schwerkraft, körper), Partikel, Game Engine (taste), Kamera (kamera_starten), KI Training (ki_erstellen, ki_lernen), Mehrspieler (mehrspieler_senden/lesen), Shader (shader(glsl)).`;

// ── SPEECH TO CODE ───────────────────────────────────────────────────
function speechToCode(raw){let t=raw.trim();
const kws=["drucke","wenn","sonst","solange","für","funktion","zurückgeben","wahr","falsch",
"und","oder","nicht","nichts","bereich","länge","zeichenkette","ganzzahl","liste",
"zeichne_kreis","zeichne_linie","zeichne_rechteck","fülle","sinus","kosinus","zufall",
"schildkröte","spiele_ton","körper","schwerkraft","partikel_explosion","taste","spielobjekt",
"szene_3d","würfel_3d","kugel_3d","kamera_starten","ki_erstellen","ki_lernen",
...
const nums=[["null","0"],["eins","1"],["zwei","2"],["drei","3"],["vier","4"],["fünf","5"],
["sechs","6"],["sieben","7"],["acht","8"],["neun","9"],["zehn","10"]];
for(const[w,d]of nums)t=t.replace(new RegExp(`\\b${w}\\b`,"gi"),d);
t=t.replace(/\bgleich gleich\b/gi,"==").replace(/\bgrößer\b/gi,">").replace(/\bkleiner\b/gi,"<")
...
if(/^(für\s+\w+\s+in|wenn\s|solange\s|funktion\s)/.test(t)&&!t.endsWith(":"))t+=":";
return t.replace(/\s+/g," ").trim()}

// ── GALLERY ──────────────────────────────────────────────────────────
const GALLERY=[
{t:"🧊 3D Welt",d:"Three.js 3D Szene",c:`# 3D-Szene mit deutscher API!
szene_3d("#0a0a1a")
...
für i in bereich(0, 5):
    für j in bereich(0, 5):
        höhe = 0.3 + zufall_dezimal() * 2
        x = (i - 2) * 2.2
        z = (j - 2) * 2.2
        farbe = "hsl(" + zeichenkette(i * 50 + j * 30) + ",60%,45%)"
        würfel_3d(x, höhe / 2 - 1, z, höhe, farbe)
...
{t:"🖐️ Kamera",d:"Körpersteuerung",c:`# Webcam-Bewegungssteuerung!
...
funktion zeichne():
    fülle("#08080f")
    # Bewegungsanzeige
    größe = 10 + kamera_bewegung * 2
    farbe = "hsl(" + zeichenkette(kamera_bewegung * 3) + ",80%,55%)"
    zeichne_kreis(kamera_x, kamera_y, größe, farbe)
    # Spur
    zeichne_kreis(kamera_x, kamera_y, größe + 15, farbe + "30")
    zeichne_kreis(kamera_x, kamera_y, größe + 30, farbe + "15")
    # Info
    zeichne_text(10, 20, "🖐️ Bewege dich vor der Kamera!", "#666", 13)
...
{t:"🧠 KI-Train",d:"KI Trainieren",c:`# Trainiere deine eigene KI!
ki = ki_erstellen()

# Trainings-Daten: Tiere nach [Größe, Geschwindigkeit, Gewicht]
...
# Testen
drucke("🧠 KI-Modell trainiert!")
drucke("Daten: " + zeichenkette(ki_daten_anzahl(ki)) + " Einträge")
drucke("Klassen: " + zeichenkette(ki_klassen(ki)))
drucke("Genauigkeit: " + zeichenkette(ki_genauigkeit(ki)) + "%")
drucke("")
drucke("--- Vorhersagen ---")
drucke("Klein+Langsam → " + ki_vorhersagen(ki, [0.1, 0.2, 0.03]))
drucke("Mittel+Schnell → " + ki_vorhersagen(ki, [0.5, 0.6, 15.0]))
drucke("Groß+Schnell → " + ki_vorhersagen(ki, [1.9, 0.88, 480.0]))

# Visualisierung
klassen = ki_klassen(ki)
farben = ["#f43f5e", "#38bdf8", "#facc15", "#2dd4bf"]
für k in bereich(0, länge(klassen)):
    zeichne_rechteck(20, 240 + k * 35, 360, 28, farben[k] + "20")
    zeichne_text(30, 260 + k * 35, klassen[k], farben[k], 16)
zeichne_text(100, 220, "🧠 Trainierte Klassen", "#666", 14)`},

{t:"⚛️ Physik",d:"Physik-Engine",c:`# Physik-Engine: Bälle mit Schwerkraft
schwerkraft(0.4)
körper(0, 380, 400, 20, {"fest": wahr, "farbe": "#1a1a2e"})
körper(100, 300, 150, 10, {"fest": wahr, "farbe": "#38bdf8"})
für i in bereich(0, 8):
    x = 50 + i * 45
    farbe = "hsl(" + zeichenkette(i * 40) + ",75%,55%)"
    körper(x, 20 + i * 15, 18, 18, {"farbe": farbe, "rückprall": 0.6 + i * 0.04})
funktion zeichne():
    fülle("#08080f")
    physik_schritt()
    alle_körper_zeichnen()
    zeichne_text(10, 18, "⚛️ Physik-Engine", "#525280", 13)`},

{t:"🎮 Pong",d:"Tastatur-Spiel",c:`# PONG — Pfeiltasten ↑↓
schläger = spielobjekt(20, 170, 12, 60, "#f43f5e")
...
funktion zeichne():
    ...
    wenn ball.y < 0 oder ball.y > 390:
        ball.vy = ball.vy * -1
    wenn objekte_kollidieren(ball, schläger):
...
{t:"🐢 Stern",d:"Schildkröte",c:`# Schildkröte zeichnet einen Stern
schildkröte.farbe("#facc15")
schildkröte.breite(3)
für i in bereich(0, 5):
    schildkröte.vorwärts(150)
    schildkröte.rechts(144)`},

{t:"🌀 Spirale",d:"Regenbogen",c:`schildkröte.breite(2)
für i in bereich(0, 150):
    farbe = "hsl(" + zeichenkette(i * 2.4) + ",80%,60%)"
    schildkröte.farbe(farbe)
    schildkröte.vorwärts(i * 1.5)
    schildkröte.rechts(91)`},

{t:"🌳 Fraktal",d:"Rekursiver Baum",c:`# Fraktalbaum — rekursive Verzweigung
funktion ast(tiefe, länge, winkel):
    wenn tiefe == 0:
        zurückgeben nichts
    # Äste werden dünner und farbiger
    schildkröte.breite(tiefe * 0.8)
    grün = ganzzahl(100 + (8 - tiefe) * 20)
    schildkröte.farbe("rgb(60," + zeichenkette(grün) + ",40)")
    wenn tiefe < 3:
        schildkröte.farbe("#2dd4bf")
    schildkröte.vorwärts(länge)
    # Links verzweigen
    schildkröte.links(winkel)
    ast(tiefe - 1, länge * 0.72, winkel)
    # Rechts verzweigen
    schildkröte.rechts(winkel * 2)
    ast(tiefe - 1, länge * 0.72, winkel)
    # Zurück
    schildkröte.links(winkel)
    schildkröte.rückwärts(länge)

# Baum zeichnen
schildkröte.stift_hoch()
schildkröte.gehe_zu(200, 380)
schildkröte.stift_runter()
schildkröte.links(90)
ast(8, 65, 28)`},

{t:"🪐 Orbit",d:"Animation",c:`funktion zeichne():
    fülle("#050510")
    für i in bereich(0, 40):
        sx = (i * 137 + rahmen * 0.2) % 400
        sy = (i * 89) % 400
        zeichne_kreis(sx, sy, 0.8, "#ffffff20")
    zeichne_kreis(200, 200, 25, "#facc15")
    für p in bereich(0, 6):
        abst = 55 + p * 32
...
{t:"📊 Sortier",d:"Bubblesort Live",c:`# Bubblesort — Schritt für Schritt animiert
werte = liste()
für i in bereich(0, 50):
    werte.anhängen(zufall(10, 380))
...
funktion zeichne():
    ...
    wenn nicht fertig:
        ...
        wenn werte[zeiger] > werte[zeiger + 1]:
            ...
            tausch = tausch + 1
        zeiger = zeiger + 1
        wenn zeiger >= länge(werte) - 1 - durch:
            zeiger = 0
            durch = durch + 1
            wenn durch >= länge(werte) - 1:
                fertig = wahr
    # Balken zeichnen
    b = 400 / länge(werte)
    für i in bereich(0, länge(werte)):
        farbe = "#38bdf8"
    ...
    zeichne_text(10, 18, "Bubblesort — Tausch: " + zeichenkette(tausch), "#e2e2f0", 12)
    wenn fertig:
        zeichne_text(130, 200, "✓ Sortiert!", "#2dd4bf", 18)`},

{t:"👥 Multi",d:"Mehrspieler",c:`# Mehrspieler — öffne dieses Artifact in 2 Tabs!
...
funktion zeichne():
    ...
    # Info
    zeichne_text(10, 18, "👥 Spieler: " + zeichenkette(mehrspieler_anzahl()), "#666", 12)
    zeichne_text(10, 36, "Öffne in 2 Tabs!", "#525280", 10)`},

{t:"🌐 Shader",d:"Julia-Fraktal",c:`# GPU Julia-Fraktal — Maus bewegt den Parameter!
shader("precision highp float; uniform float zeit; uniform vec2 auflösung; uniform vec2 maus; void main(){ vec2 uv=(gl_FragCoord.xy-auflösung*0.5)/auflösung.y*2.5; vec2 c=vec2(cos(zeit*0.3)*0.7,sin(zeit*0.23)*0.7); if(maus.x>0.0)c=(maus/auflösung-0.5)*2.5; vec2 z=uv; float i; for(i=0.0;i<128.0;i++){if(dot(z,z)>4.0)break; z=vec2(z.x*z.x-z.y*z.y,2.0*z.x*z.y)+c;} float t=i/128.0; float r=0.5+0.5*cos(3.0+t*6.28+0.0); float g=0.5+0.5*cos(3.0+t*6.28+2.1); float b=0.5+0.5*cos(3.0+t*6.28+4.2); if(i>=128.0){r=0.0;g=0.0;b=0.0;} gl_FragColor=vec4(r,g,b,1.0); }")

funktion zeichne():
    x = 0`},

{t:"🧬 Life",d:"Conway's Game of Life",c:`# Conway's Game of Life — Zellulärer Automat
größe = 40
zellen = liste()
nächste = liste()

# Zufälliges Startmuster
für y in bereich(0, größe):
    zeile = liste()
    z2 = liste()
    für x in bereich(0, größe):
        zeile.anhängen(zufall(0, 4) == 0)
        z2.anhängen(falsch)
    zellen.anhängen(zeile)
    nächste.anhängen(z2)

funktion nachbarn(gx, gy):
    n = 0
    für dy in bereich(-1, 2):
        für dx in bereich(-1, 2):
            wenn dx == 0 und dy == 0:
                weiter
            nx = (gx + dx) % größe
            ny = (gy + dy) % größe
...
funktion zeichne():
    ...
    für y in bereich(0, größe):
        für x in bereich(0, größe):
            n = nachbarn(x, y)
            wenn zellen[y][x]:
                nächste[y][x] = (n == 2 oder n == 3)
            sonst:
                nächste[y][x] = (n == 3)
    # Kopieren und zeichnen
    für y in bereich(0, größe):
        für x in bereich(0, größe):
            zellen[y][x] = nächste[y][x]
            wenn zellen[y][x]:
...
export default function BlackSwan() {
  ...
  // ── CANVAS MOUSE + TOUCH (overlay approach for all modes) ────────
  ...
  const startCamera = useCallback(async ()=>{
    ...
    try{
      ...
      setCamActive(true);
    }catch(e){console.log("Kamera nicht verfügbar");}
  },[]);
  ...
  const renderShader = useCallback((fragSrc,time)=>{
    ...
    const rLoc=gl.getUniformLocation(prog,"auflösung");if(rLoc)gl.uniform2f(rLoc,400,400);
  ...
  const runCode = useCallback(()=>{
    ...
    try{
      ...
      const has3D=/szene_3d|würfel_3d|kugel_3d|zylinder_3d|ebene_3d/.test(code);
...
let rahmen=0,maus_x=0,maus_y=0,maus_gedrückt=false;
${js}
return{zeichne:typeof zeichne==='function'?zeichne:null,
  setMouse(x,y,d){maus_x=x;maus_y=y;maus_gedrückt=d},
  setFrame(f){rahmen=f},
  ...
  getTurtle(){return __turtle}};`;
        ...
        if(ctx.zeichne){
          ...
          const animate=()=>{
            ...
            try{ctx.zeichne();}catch(e){setConsoleOut([{ty:"err",t:"⚠ "+tErr(e.message)}]);stopAnim();return;}
          ...
          setConsoleOut([{ty:"sys",t:`🔄 Animation läuft... ${features.length?`[${features.join("+")}]`:""} (${code.split("\n").length} Zeilen)`}]);
        }
      }else{
        ...
        const fn=new Function("__out","__err","__d","__audioCtx","__keys","__cam","__storage","rahmen","maus_x","maus_y","maus_gedrückt",fullCode);
        ...
        errors.forEach(e=>lines.push({ty:"err",t:"⚠ "+tErr(e)}));
        if(!lines.length)lines.push({ty:"sys",t:"✓ "+(result.shader_mode?"Shader aktiv":result.mode3d?result.d3.length+" 3D-Objekte":draws.length+" Zeichenbefehle")});
        setConsoleOut(lines);

        if(showDebug){
          const vars={};
          const vm=code.matchAll(/^(\s*)([\wäöüÄÖÜß]+)\s*=\s*(.+)$/gm);
          for(const m of vm){if(!["funktion","für","wenn","solange","sonst","klasse"].includes(m[2]))vars[m[2]]=m[3].trim();}
          setDebugVars(vars);
        }
      }
    }catch(e){setConsoleOut([{ty:"err",t:"⚠ "+tErr(e.message)}]);renderCanvas([],null);}
  },[code,stopAnim,renderCanvas,render3D,renderShader,showDebug,startCamera,processCamera,startMPPoll]);

  // Auto-run
  useEffect(()=>{
    if(/^funktion zeichne\(\)\s*:/m.test(code)){setConsoleOut([{ty:"sys",t:"🔄 Animation erkannt — drücke ▶ Ausführen"}]);return;}
    stopAnim();runCode();
  },[code]);
  ...
  const modeLabel=isShader?"🌐 SHADER":is3D?"🧊 3D":(/taste\(/.test(code)&&isAnim)?"🎮 GAME":camActive?"🖐️ CAM":(/mehrspieler_/.test(code)&&isAnim)?"👥 MULTI":isAnim?"🔄 LIVE":"● Bereit";
...
@keyframes glowShader{0%,100%{box-shadow:0 0 15px ${C.P}15}50%{box-shadow:0 0 30px ${C.P}25}}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        ...
        <div style={{display:"flex",gap:3,alignItems:"center",flexWrap:"wrap"}}>
          <button onClick={isAnim?stopAnim:runCode} style={{padding:"4px 12px",borderRadius:14,border:"none",
            ...
            color:"#fff",fontFamily:"'Outfit'",fontSize:10,fontWeight:700}}>{isAnim?"⏹ Stopp":"▶ Ausführen"}</button>
          {voiceOK&&<Btn onClick={()=>{setShowVoice(true);startVoice()}} color={C.O}>🎤</Btn>}
          <Btn onClick={()=>setShowAI(true)} color={C.P}>🤖 KI</Btn>
          <div style={{width:1,height:14,background:C.BD+"40",margin:"0 1px"}}/>
          <Btn onClick={()=>togglePanel("py")} active={showPy} color={C.B}>🐍</Btn>
          <Btn onClick={()=>togglePanel("debug")} active={showDebug} color={C.Y}>🔄</Btn>
          <Btn onClick={()=>togglePanel("tutor")} active={showTutor} color={C.T}>🧠</Btn>
          <Btn onClick={()=>{const cv=isShader?shaderRef.current:is3D?canvas3DRef.current:canvasRef.current;
            if(!cv)return;const a=document.createElement("a");a.download="black-swan.png";a.href=cv.toDataURL();a.click();}} color={C.DM} title="Canvas speichern">💾</Btn>
        </div>
      </div>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div style={{width:46,background:C.S1,borderRight:`1px solid ${C.BD}30`,
          ...
          {GALLERY.map((g,i)=>(
            <button key={i} onClick={()=>{setActiveG(i);stopAnim();setCode(g.c)}} title={g.t+" — "+g.d}
        ...
        {sidePanel&&(
          <div style={{width:220,borderRight:`1px solid ${C.BD}30`,flexShrink:0,
            ...
            {showPy&&(<>
              <div style={{padding:"4px 10px",fontSize:7,letterSpacing:2,color:C.B,fontFamily:"'Outfit'",fontWeight:700,borderBottom:`1px solid ${C.BD}30`}}>🐍 PYTHON</div>
            ...
            {showDebug&&(<>
              <div style={{padding:"4px 10px",fontSize:7,letterSpacing:2,color:C.Y,fontFamily:"'Outfit'",fontWeight:700,borderBottom:`1px solid ${C.BD}30`}}>🔄 VARIABLEN</div>
              <div style={{flex:1,overflow:"auto",padding:"6px 8px",fontSize:9.5}}>
                {Object.keys(debugVars).length===0?
                  <div style={{color:C.DM,fontFamily:"'Outfit'",fontSize:10,padding:6}}>Führe Code aus um Variablen zu sehen.</div>:
                ...
                <div style={{marginTop:10,padding:"5px 7px",borderRadius:5,background:C.S1,border:`1px solid ${C.BD}30`,fontSize:8,color:C.DM}}>
                  ...
                  <div>👥 mehrspieler_senden/lesen</div>
                  <div>🌐 shader · shader_wert</div>
                  <div>🧊 szene_3d · würfel_3d · kugel_3d</div>
                  <div>🖐️ kamera_starten · kamera_x/y</div>
                  <div>🧠 ki_erstellen · ki_lernen</div>
                  <div>⚛️ schwerkraft · körper · kraft</div>
                  <div>🎭 partikel_explosion</div>
                  <div>🎮 taste · spielobjekt · bewege</div>
                </div>
              </div>
            </>)}
            {showTutor&&(<>
              <div style={{padding:"4px 10px",fontSize:7,letterSpacing:2,color:C.T,fontFamily:"'Outfit'",fontWeight:700,borderBottom:`1px solid ${C.BD}30`}}>🧠 KI-TUTOR</div>
              <div style={{flex:1,overflow:"auto",padding:"6px 8px",display:"flex",flexDirection:"column",gap:6}}>
                <div style={{display:"flex",flexWrap:"wrap",gap:2}}>
                  {["Was macht das?","Verbesserungen?","Zeile für Zeile","Fehler?"].map(q=>(
                ...
                {!tutorA&&!tutorLoading&&<div style={{color:C.DM,fontFamily:"'Outfit'",fontSize:9.5,padding:4}}>Frage den KI-Tutor! 🇩🇪</div>}
              </div>
            </>)}
          </div>
        )}
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
          ...
          {/* Canvas wrapper — contains all canvases + mouse overlay */}
          <div style={{position:"relative",width:400,height:400,maxWidth:"100%",maxHeight:"calc(100vh - 80px)",zIndex:1}}>
            ...
            {/* Mouse/Touch overlay — captures events for ALL canvas modes */}
          ...
          {camActive&&<div style={{position:"absolute",top:8,left:12,
            ...
            animation:"pulse 1.5s infinite"}}>📷 Kamera aktiv</div>}
          {/* Game hint */}
          {isAnim&&/taste\(/.test(code)&&!is3D&&(
            <div style={{position:"absolute",bottom:8,right:12,padding:"3px 8px",borderRadius:8,
              background:`${C.S1}cc`,border:`1px solid ${C.BD}40`,fontSize:7,color:C.DM,fontFamily:"'Outfit'",zIndex:2}}>⌨️ Pfeiltasten</div>)}
        </div>
      </div>
      ...
      {showVoice&&(
        <div style={{position:"absolute",inset:0,background:`${C.BG}f5`,backdropFilter:"blur(20px)",zIndex:100,
          ...
          <button onClick={()=>{stopVoice();setShowVoice(false)}} style={{position:"absolute",top:12,right:12,width:28,height:28,borderRadius:"50%",border:`1px solid ${C.BD}`,background:C.S1,color:C.DM,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          <div style={{fontFamily:"'Outfit'",fontSize:22,fontWeight:900,background:`linear-gradient(135deg,${C.R},${C.O})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{listening?"Ich höre zu...":"Sprechen"}</div>
          ...
          {(transcript||interim)&&(
            <div style={{maxWidth:440,width:"100%",padding:"10px 14px",borderRadius:8,background:`${C.S1}cc`,border:`1px solid ${C.BD}`}}>
              ...
              <div style={{marginTop:4,fontSize:10,color:C.O,fontFamily:"'Fira Code'"}}>→ {speechToCode(transcript||interim)}</div>
            </div>)}
          <div style={{display:"flex",gap:8}}>
            {listening
              ?<button onClick={stopVoice} style={{width:52,height:52,borderRadius:"50%",border:"none",cursor:"pointer",background:C.R,color:"#fff",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",animation:"breathe 1.5s ease infinite"}}>⏹</button>
              :<button onClick={startVoice} style={{width:52,height:52,borderRadius:"50%",border:`2px solid ${C.R}40`,cursor:"pointer",background:C.S1,color:C.R,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>🎤</button>}
          </div>
          {transcript&&(
            <div style={{display:"flex",gap:5}}>
              <button onClick={()=>{const c=speechToCode(transcript);setCode(p=>p+(p&&!p.endsWith("\n")?"\n":"")+c);stopVoice();setShowVoice(false)}}
                style={{padding:"6px 14px",borderRadius:6,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${C.O},${C.R})`,color:"#fff",fontFamily:"'Outfit'",fontSize:10,fontWeight:700}}>✍️ Einfügen</button>
              <button onClick={()=>{sendToAI(transcript);stopVoice();setShowVoice(false)}}
                style={{padding:"6px 14px",borderRadius:6,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${C.P},${C.B})`,color:"#fff",fontFamily:"'Outfit'",fontSize:10,fontWeight:700}}>🤖 An KI</button>
            </div>)}
        </div>)}
      ...
      {showAI&&(
        <div style={{position:"absolute",inset:0,background:`${C.BG}f5`,backdropFilter:"blur(20px)",zIndex:100,
          ...
          <button onClick={()=>setShowAI(false)} style={{position:"absolute",top:12,right:12,width:28,height:28,borderRadius:"50%",border:`1px solid ${C.BD}`,background:C.S1,color:C.DM,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,justifyContent:"center",maxWidth:500}}>
            {["3D-Stadt mit Kugeln","Physik Bälle Demo","Partikel-Feuerwerk",
              "Pong-Spiel","Kamera-Steuerung","KI die Farben lernt",
              "Mehrspieler Cursor-Jagd","Plasma-Shader","Schildkröte Mandala","Planetensystem"].map(s=>(
          ...
          <div style={{display:"flex",gap:4,width:"100%",maxWidth:460}}>
            ...
            <button onClick={()=>sendToAI(aiInput)} disabled={aiLoading}
              ...
              {aiLoading?<span style={{animation:"spin .8s linear infinite",display:"inline-block"}}>⏳</span>:"→"}</button>
          </div>
        </div>)}
      ...
      <div style={{height:18,display:"flex",alignItems:"center",justifyContent:"space-between",
        ...
        <div style={{display:"flex",gap:6,fontFamily:"'Outfit'"}}>
          ...
          {camActive&&<span style={{color:C.O}}>📷</span>}
        </div>
        <span style={{letterSpacing:0.3}}>👥Multi · 🌐Shader · 🧊3D · 🖐️Körper · 🧠KI · ⚛️Physik · 🎭Partikel · 🎮Game · 🐢Turtle · 🔊Sound · 📱Touch</span>
      </div>
    </div>
  );
}
